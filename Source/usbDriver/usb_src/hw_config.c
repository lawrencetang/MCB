...
void USB_Interrupts_Config(void)
{
  ...
  NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);//lt:一位先占优先级，3位从优先级

  NVIC_InitStructure.NVIC_IRQChannel = USB_LP_CAN1_RX0_IRQn;//lt:库文件不一致，在stm32f10x.h中做出了宏定义
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;//lt:先占优先级0
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;//lt:从优先级0
  ...
  NVIC_Init(&NVIC_InitStructure);//lt:使能串口先占优先级1
}
...
void Get_SerialNum(void)
{
  ...
  Device_Serial2 = *(__IO uint32_t*)(0x1FFFF7F0);//lt:将这三个地址的数取出赋值，再保存到virtual_com_port_stringserial中

  if (Device_Serial0 != 0)
  {
    Virtual_Com_Port_StringSerial[2] = (uint8_t)(Device_Serial0 & 0x000000FF);//lt:取低8位
