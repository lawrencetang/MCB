...
*******************************************************************************/
  ...
  const u32 SystemFrequency_APB2Clk = SYSCLK_FREQ_56MHz;      /*!< APB Peripheral bus 2 (high) speed   */  
...
static void SetSysClockTo36(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE | RCC_CFGR_PLLMULL));
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLXTPRE_HSE | RCC_CFGR_PLLMULL6);
#endif /* STM32F10X_CL */
